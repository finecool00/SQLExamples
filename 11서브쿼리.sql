...
--주의할 점, 단일행 이어야 합니다. 컬럼값도 1개여야 합니다.
SELECT *
FROM EMPLOYEES
...
--ALL 최대값보다 큼, 최소값보다 작음
SELECT * 
FROM EMPLOYEES
WHERE SALARY > ALL (SELECT SALARY FROM EMPLOYEES WHERE FIRST_NAME = 'David'); --급여가 4800보다 작은사람들
...
--ROWNUM는 조회된 순서이기 때문에, ORDER와 같이 사용되면 ROWNUM이 섞이는 문제가 발생함
--그래서 인라인뷰 구문으로 출력할 컬럼에 대하여(또는 전체) 정렬을 선행하는 것
SELECT FIRST_NAME,
       SALARY,
       ROWNUM
...
--문법2 -인라인뷰에 엘리어스 설정해놓으면 SELECT 구문에서 아스타링크(*)와 다른 컬럼(ROWNUM 함께 사용 가능)       
SELECT ROWNUM,
       A.*
FROM (SELECT FIRST_NAME,
             SALARY
