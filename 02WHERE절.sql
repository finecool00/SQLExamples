...
SELECT * FROM EMPLOYEES WHERE HIRE_DATE LIKE '%12%'; --12가 포함된(이 경우 년, 월, 일 어디에든)
...
SELECT * FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR' AND SALARY >= 6000; --FI중 6000이상인 사람 OR IT_PROG
SELECT * FROM EMPLOYEES WHERE (JOB_ID = 'IT_PROG' OR JOB_ID = 'FI_MGR') AND SALARY >= 6000; --IT 이거나 FI인 사람 중 SALARY 6000이상
...
SELECT * FROM EMPLOYEES WHERE JOB_ID IN('IT_PROG', 'ST_MAN') ORDER BY FIRST_NAME DESC; --문자(사전등재순 정렬)
