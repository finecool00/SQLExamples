...
SELECT *
FROM EMPLOYEES
WHERE SALARY > (SELECT AVG(SALARY) FROM EMPLOYEES WHERE JOB_ID = 'IT_PROG'); --5760

--문제 2.
---DEPARTMENTS테이블에서 manager_id가 100인 사람의 department_id와
--EMPLOYEES테이블에서 department_id가 일치하는 모든 사원의 정보를 검색하세요.

SELECT DEPARTMENT_ID FROM DEPARTMENTS D WHERE MANAGER_ID = 100;
...
SELECT EMPLOYEE_ID, FIRST_NAME, PHONE_NUMBER, HIRE_DATE FROM EMPLOYEES ORDER BY HIRE_DATE;
...
SELECT LOCATION_ID,
       ...
       CITY,
       COUNTRY_ID,
       (SELECT COUNTRY_NAME FROM COUNTRIES C WHERE L.COUNTRY_ID = C.COUNTRY_ID) COUNTRY
FROM LOCATIONS L;
...
--선생님 풀이(깨달은 것 : 조인을 이어붙일 때는 서브쿼리 안써도 된다!? 상황마다 다를지도)
SELECT D.*,
