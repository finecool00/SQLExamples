...
INSERT INTO DEPTS VALUES(DEPTS_SEQ.NEXTVAL, 'TEST', 100, 1000); -- X10(시퀀스의 최대값 도달하면 더이상 사용할 수 없음)
--시퀀스 수정
...
--2.증가값을 음수로 변경
ALTER SEQUENCE DEPTS_SEQ INCREMENT BY -1; 현재시퀀스 -1로 감소
--3.시퀀스 실행
...
--시퀀스 VS 년별로 시퀀스 VS 랜덤한 문자열(중복안되게) - PK를 지정할 수 있다(데이터 양이 많아질 때?!)
--20230523-00001-상품번호 -> KEY는 문자열 타입으로 해야할 것
CREATE TABLE DEPTS3 (
...
-- TO_CHAR, LPAD(자릿수, '채울값')
INSERT INTO DEPTS3 VALUES( TO_CHAR(SYSDATE, 'YYYYMMDD') || '-' || LPAD(DEPTS3_SEQ.NEXTVAL, 5, '0') || '-' || '상품번호' , 'TEST');
SELECT * FROM DEPTS3;
--------------------------------------------------------------------------------
--INDEX
...
--인덱스 생성(인덱스는 조회를 빠르게 하긴 하지만, 무작위하게 많이 생성하면, 오히려 성능이 떨어질 수 있습니다.)
