--문자열함수
--LOWER(), INITCAP(), UPPER()
SELECT LOWER('HELLO'), INITCAP('HELLO'), UPPER('HELLO') FROM DUAL; --DUAL은 가상테이블(결과 출력 해보고 싶을 때 사용)
...
SELECT LTRIM(FIRST_NAME, 'A'), FIRST_NAME FROM EMPLOYEES; --왼쪽에 처음 발견되는 문자 A제거
...
SELECT REPLACE('HELLO WORLD ~!  ', ' ', '') AS RESULT FROM DUAL; --모든공백제거
...
SELECT CONCAT('02', SUBSTR(PHONE_NUMBER, 4, 11)) FROM EMPLOYEES;
